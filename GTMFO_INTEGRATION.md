...
## 9. 农业平衡与季节联动（2026-09-14，gtmfo-0.0.7）

**背景**：实机反馈香蕉树约 2 分钟一轮（树叶果实 + GT 温室），严重破坏经济平衡。
完整代码取证与数值对照见模组仓库 `docs/AGRICULTURE_BALANCE.md`。

### 9.1 修复果实掉率 BUG（约 10~12.5 倍超标）
- `GTFOBlockLeaves#getDrops` 误用了**树苗**掉率常量 20；原版 1.12.2 `BlockLeaves#getDrops`
  传给 `dropApple` 的是 **200**（苹果 0.5%），GTFO 的 `getAppleDrop(chance)` 再除以每树 divisor。
- 已改为 200：整棵香蕉树期望产量 **~101 → ~8 个**，其余果树同比例削弱。

### 9.2 GT 温室限速（配置）
- 新增 `gtfoMiscConfig.greenhouseDurationMultiplier`（1.0~100.0，默认 1.0 保持原速）。
- 本包 `config/gtmfo.yaml` 设为 **6.0**：树木电路 100 s → **600 s（10 分钟）**、
  果实电路 150 s → 900 s（15 分钟）；EV 超频后约 75 s / 112 s。
- 需要更慢/更快直接改这个数即可（无需动配方）。

### 9.3 树苗纳入季节系统（SereneSeasons）
| 季节 | 树苗 |
|---|---|
| 春 | 杏、柠檬、青柠 |
| 夏 | 香蕉、芒果、橙、椰子 |
...
- 非当季树苗不 randomTick 生长、骨粉无效（SereneSeasons 内置规则）；
- 上方盖玻璃（任意 `#forge:glass`，含本模组温室玻璃）→ 全年生长；
- 机制依据：反编译 `SeasonalCropGrowthHandler` / `ModFertility`（方块标签判定 + `isGlassAboveBlock`）。

### 9.4 部署
- `mods/gtmfo-0.0.6.jar` → **`mods/gtmfo-0.0.7.jar`**（本地部署；含上述全部修改）。
