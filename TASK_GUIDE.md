...
- [x] 5.12 章节提交（feat commit `8d565c5`）
- [x] 5.13 更新本指南进度日志
...
| 2026-09-13 | 章节提交 `8d565c5`：30 任务 + 98×2 本地化条目 + 生成器 + 指南 |
| 2026-09-13 | 修复生成器遗漏的章节级 `title` 字段（现有章节均以 `title: "{ftbquests.chapter.<file>.title}"` 结尾） |
| 2026-09-13 | 新增 `MODPACK_REPORT.md` / `MODPACK_REPORT.json`（供其他 AI 联动：mod 分类、食物/作物/饮品/渔业清单、章节表、接口约定） |

---

## 8. 后续可选项

- 在游戏内打开任务书检查排版（`/ftbquests editing_mode` 可微调坐标）。
- 如需让章节更“硬核”：可把 `checkmark` 任务换成 `observation`（击杀首领）或 `advancement` 任务。
- 如需联动包内经济：可在生成器 `QUESTS` 中调整 `rewards` 后重新运行脚本（ID 保持不变）。
- 语言文件中的其它语言（ja_jp/ko_kr 等）未同步；如需可扩展生成器一次输出全部语言。
